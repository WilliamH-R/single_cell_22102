...
```{r}
raw_data <- raw_data |>
  RenameGenesSeurat(newnames = unfactor(raw_data@assays[["RNA"]]@meta.features[["feature_name"]]))
```

# Remove leftover ENSG genes
```{r}
# Extract used genes
genes_used <- stringr::str_extract(unfactor(raw_data@assays[["RNA"]]@meta.features[["feature_name"]]),
                                   pattern = "^(?!ENSG).+") |>
  na.omit()
