...
```{r}
ribo_genes <- readr::read_tsv(file = "https://www.genenames.org/cgi-bin/genegroup/download?id=1054&type=branch",
                              show_col_types = FALSE) |>
  filter(stringr::str_detect(string = `Approved name`,
                             pattern = "^ribosomal")) |>
  pull(`Approved symbol`)
...
```{r}
raw_data <- raw_data |>
  subset(subset = nFeature_RNA > XXX & # Is 250 in paper
           nFeature_RNA < XXX &
           nCount_RNA > XXX &
           nCount_RNA < XXX &
           mito_DTU < XXX &
