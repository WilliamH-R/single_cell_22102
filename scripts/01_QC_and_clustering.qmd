...
Do some cells really have almost 100 % HB expression.
Interrogating those cells
```{r}
## Getting cell ID for top 5 HB expressing cells
library(dplyr)
top_hb <- meta_data |> 
  arrange(desc(hemo_DTU)) |> 
  head() |> 
  rownames()

## Would be nice to get a count matrix for all cells
counts_mat <-GetAssayData(object = raw_data, assay = "RNA", layer = "counts")
```
...
# Find clusters
```{r}
raw_data <- FindNeighbors(raw_data, dims = 1:42)
...
```{r}
raw_data <- RunUMAP(object = raw_data, dims = 1:n_dimensions)
```
Making the UMAP plot
```{r}
DimPlot(object = raw_data, reduction = "umap")
```
