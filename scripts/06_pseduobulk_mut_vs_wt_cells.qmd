...
gsea_bound <- rbind(gsea_bottom10, gsea_top10) |> 
  mutate(sign = case_when(padj < 0.05 ~ "Sign.", 
                          padj >  0.05 ~ "Non-sign"))
```
...
# CD4+ T cells DGE analysis
Getting the count data for monocytes

```{r}
mono_counts <- cts.split.modified$`CD4+ T cells`
...
ggplot(data = res_df, 
  ...
  theme_bw() +
  labs(title = "Volcano plot of differentially expresssed genes by\nTET2mut vs. TET2wt monocytes",
       subtitle = "Highlighted genes have p-adjusted < 0.05") +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5)) +
  theme(legend.position = "null")
```
...
# CD8+ T cells DGE analysis
Getting the count data for monocytes

```{r}
mono_counts <- cts.split.modified$`CD8+ T cells`
...
# NK cells DGE analysis
Getting the count data for monocytes

```{r}
mono_counts <- cts.split.modified$`NK cells`
