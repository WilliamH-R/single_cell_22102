...
resolution <- c(0.025, 0.05, 0.1, 0.2)
...
# Set idents
Choose idents that gives xxx clusters (this is the number of cell types we expect to have). We use `"Harmony_Integration_subset"` reduction to use the integrated data.
```{r}
...
for (res in resolution) {
  p <- DimPlot(data,
               ...
               label = TRUE)
  print(p)
  ggsave(p, filename = glue::glue("../results/cluster_resolution_{res}.png"))
}
...
DimPlot(data,
        ...
        label = TRUE)
ggsave(filename = "../results/cell_types.png")

DimPlot(data,
        reduction = reduction_to_use,
        group.by = "Clone",
        label = TRUE)
ggsave(filename = "../results/clone.png")
DimPlot(data,
        reduction = reduction_to_use,
        group.by = "disease",
        label = TRUE)
ggsave(filename = "../results/disease.png")
DimPlot(data,
        reduction = reduction_to_use,
        group.by = "MUTATION.GROUP_Clone",
        label = TRUE)
ggsave(filename = "../results/MUTATION_GROUP_Clone.png")
DimPlot(data,
        reduction = reduction_to_use,
        group.by = "donor_disease",
        label = TRUE)
ggsave(filename = "../results/donor_disease.png")
```

```{r}
# Assuming your data variables are defined as in the example
contingency_table <- table(data$donor_disease,
                           data@meta.data$RNA_snn_res.0.1) |>
  as.data.frame()
...
plotGeneCount(curves,
              ...
              clusters = clustering,
              models = sce)
ggsave(filename = "../results/slingshot.png")
```
...
plot_differential_expression <- function(feature_id) {
  plots <- patchwork::wrap_plots(
    plotGeneCount(curves,
                  ...
                  models = sce,
                  gene = feature_id) +
      guides(colour=guide_legend(title = glue::glue("logged count of
                                                    gene {feature_id}"))),
    plotSmoothers(sce,
                  counts,
                  gene = feature_id)
...
plot_differential_expression("LST1")
ggsave(filename = "../results/slingshot_LST1.png")
```
